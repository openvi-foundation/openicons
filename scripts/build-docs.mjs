...
.sheet .tags { color: var(--muted); font-size: 12px; margin-bottom: 1rem; }
.preview { display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem; }
.preview svg { fill: var(--icon, currentColor); }
.snippet {
    display: flex;
    gap: 0.5rem;
    align-items: center;
...
<dialog id="sheet">
<div class="sheet">
<div class="preview"><svg id="d-svg" viewBox="0 0 24 24" width="48" height="48" aria-hidden="true"></svg>
<div><h2 id="d-name"></h2><div class="tags" id="d-tags"></div></div>
</div>
...
<div class="snippet"><code id="d-code"></code><button type="button" data-copy="d-code">Copy</button></div>
<p class="hint">Snippets follow the size and color set in Options.</p>
<button class="close" type="button" id="d-close">Close</button>
</div>
</dialog>
...
results.addEventListener('click', (event) => {
    ...
    fields.tags.textContent = card.dataset.terms.split(' ').filter((t) => t !== name).join(', ');
